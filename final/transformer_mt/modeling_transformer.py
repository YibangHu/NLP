...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.self_attention = MultiHeadAttention(input_size=hidden, hidden=hidden, num_heads=num_heads, causal=True)
        self.cross_attention = MultiHeadAttention(input_size=hidden, hidden=hidden, num_heads=num_heads, causal=False)
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        x1 = self.self_attention(q=decoder_hidden_states)
        x = x1 + decoder_hidden_states
        x = self.self_att_layer_norm(x)
        x2 = self.cross_attention(q=x, kv=encoder_hidden_states, key_padding_mask=key_padding_mask)
        x = x2 + x
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        """
        super().__init__()
        self.pre_trained_model = AutoModel.from_pretrained("DeepPavlov/rubert-base-cased")
        self.hidden = self.pre_trained_model.config.hidden_size

        self.num_layers = 6#self.pre_trained_model.config.num_hidden_layers
        self.num_heads = 8#self.pre_trained_model.config.num_attention_heads
        ...
        self.decoder_layers = nn.ModuleList(TransformerDecoderLayer(hidden=hidden, num_heads=num_heads, fcn_hidden=fcn_hidden, dropout=dropout)
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        x = self.pre_trained_model(input_ids=input_ids, attention_mask=key_padding_mask, output_hidden_states=True)
        encoder_hidden_states = x["hidden_states"][-1]
        # YOUR CODE ENDS HERE
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        x = self.decoder_embeddings(decoder_input_ids)
        decoder_hidden_states = self._add_positions(x)
        for layer in self.decoder_layers:
            decoder_hidden_states = layer(decoder_hidden_states=decoder_hidden_states, encoder_hidden_states=encoder_hidden_states, key_padding_mask=key_padding_mask)
